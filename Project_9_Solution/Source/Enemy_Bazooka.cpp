...
void Enemy_Bazooka::Update()
{
	...
	if (shootCooldown > 233 && deleting == false)
	{
		...
		case LEFT:
			...
			break;
		case RIGHT:
			App->particles->AddParticle(App->particles->missileRight, position.x, position.y, dirx, diry, false, Collider::Type::MISSILE, 0, true, 20, 20, 70);
			break;
		case DOWN:
			App->particles->AddParticle(App->particles->missileDown, position.x, position.y, dirx, diry, false, Collider::Type::MISSILE, 0, true, 20, 20, 0, 65);
			break;
		case UP:
		...
		case UPLEFT:
			...
			break;
		case UPRIGHT:
			App->particles->AddParticle(App->particles->missileUpRight, position.x, position.y, dirx, diry, false, Collider::Type::MISSILE, 0, true, 20, 20, 35);
			break;
		}
